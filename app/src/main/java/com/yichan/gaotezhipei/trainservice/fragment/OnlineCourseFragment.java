...
import com.yichan.gaotezhipei.trainservice.view.OnlineCourseAdapter;
...
public class OnlineCourseFragment extends CommonLinearListFragment {
    ...
    private List<OnlineCoursePage.BeanListBean> mList = new ArrayList<>();
    private OnlineCourseAdapter mAdapter;
    ...
    protected RecyclerView.Adapter getAdapter() {
        mAdapter = new OnlineCourseAdapter(mList, getActivity());
        return mAdapter;
    }
    @Override
    ...
    private void getDataList(int currentPage, final boolean isRefresh) {
        ...
        RequestCall call = new PostFormBuilder().url(AppConstants.BASE_URL + TrainServiceConstants.URL_GET_ONLINE_CLASS)
                .addParams("page", String.valueOf(currentPage))
                .build();
        call.doScene(new TokenSceneCallback<OnlineCoursePage>(call) {
            @Override
            ...
            @Override
            protected void handleResponse(Result<OnlineCoursePage> response) {
            ...
            public Result parseNetworkResponse(Response response) throws IOException {
                Type type = TypeBuilder
                        ...
                        .beginSubType(OnlineCoursePage.class)
                        .endSubType().build();
